...
function EntryForm({initial,onSave,onClose,departments,conferences,onAddDepartment,onAddConference,weights,weightLabels,mustHaves,folders=[],quickMode=false}) {
  ...
  return (
    <div>
      ...
      {tab==="basic"&&(
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
          ...
          <div style={{gridColumn:"1/-1"}}>
            ...
            {folders.length===0&&<div style={{fontSize:11,color:"#334",marginTop:4}}>Create folders in the Folders view to organize fintechs by problem/theme.</div>}
          </div>
        </div>
      )}
        ...
        <div>
          ...
          {(()=>{
            ...
            return (
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10,padding:"6px 10px",background:Math.abs(tot-100)<1?"rgba(0,212,170,0.06)":"rgba(231,76,60,0.06)",borderRadius:6,border:`1px solid ${Math.abs(tot-100)<1?"rgba(0,212,170,0.2)":"rgba(231,76,60,0.2)"}`}}>
                ...
                <span style={{fontSize:11,color:"#445"}}>Scoring criteria: {Object.keys(activeWeights).length}</span>
              </div>
            );
          })()}
          ...
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 72px 60px 72px 32px",gap:6,marginBottom:6,padding:"0 6px"}}>
            {["Criteria Name","Rating (1–5)","Weight %","Slider","Must-Have",""].map(h=>(
              <div key={h} style={{fontSize:9,color:"#445",textTransform:"uppercase",letterSpacing:"0.07em"}}>{h}</div>
            ))}
          </div>
          ...
          {Object.keys(activeWeights).map(k=>{
            ...
            return (
              <div key={k} style={{display:"grid",gridTemplateColumns:"1fr 1fr 72px 60px 72px 32px",gap:6,alignItems:"center",marginBottom:8,padding:"8px 8px",background:"rgba(255,255,255,0.03)",borderRadius:8,border:"1px solid rgba(255,255,255,0.06)"}}>
                ...
                <div>
                  {isEditingLabel ? (
                    ...
                    <div style={{display:"flex",alignItems:"center",gap:4}}>
                      <span style={{fontSize:13,color:"#c8d8e8",flex:1,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{(activeWeightLabels||{})[k]||k}</span>
                      <button onClick={()=>{setVendorEditingLabel(k);setVendorEditingLabelVal((activeWeightLabels||{})[k]||k);}}
                        style={{background:"none",border:"none",color:"#445",cursor:"pointer",fontSize:11,padding:"2px",flexShrink:0}} title="Rename">✎</button>
                    </div>
                  )}
                </div>
                ...
                <div style={{display:"flex",gap:3}}>
                  {[1,2,3,4,5].map(n=>(
                    <button key={n} onClick={()=>set(k,n)} style={{
                      ...
                      borderColor:form[k]===n||(!form[k]&&n===3)?"#00d4aa":"rgba(255,255,255,0.1)",
                ...
                <div style={{display:"flex",alignItems:"center",gap:2}}>
                  <input type="number" min="0" max="100"
                    ...
                    style={{width:44,background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.15)",borderRadius:4,padding:"4px 4px",color:"#00d4aa",fontSize:12,fontWeight:700,textAlign:"center"}}/>
          ...
          <div style={{padding:12,background:"rgba(0,212,170,0.04)",border:"1px dashed rgba(0,212,170,0.2)",borderRadius:8}}>
            <div style={{fontSize:11,color:"#8899aa",textTransform:"uppercase",letterSpacing:"0.08em",marginBottom:8}}>Add Criteria to This Vendor</div>
            <div style={{display:"flex",gap:6}}>
            ...
            <div style={{fontSize:10,color:"#334",marginTop:4}}>New criteria are added to this vendor only. To add globally, use the Weights tab.</div>
          </div>
        </div>
      )}
      ...
      {tab==="notes"&&(
        <div>
          {/* Structured conference capture fields */}
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12}}>
              {NOTE_FIELDS.map(({key,label,placeholder})=>(
                <div key={key} style={{gridColumn:key==="notePitchSummary"||key==="noteRedFlags"||key==="noteFollowUp"?"1/-1":"auto"}}>
                  <label style={{...css.label,color:"#7799aa"}}>{label}</label>
      ...
      {/* Autosave status bar */}
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginTop:20,paddingTop:14,borderTop:"1px solid rgba(255,255,255,0.06)"}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          {autoSaveStatus==="idle"&&form.fintechName&&(
            <div style={{fontSize:11,color:"#334"}}>Autosaves 10s after each change</div>
          )}
          {!form.fintechName&&(
            <div style={{fontSize:11,color:"#334"}}>Enter a Fintech Name to enable autosave</div>
          )}
        </div>
        <div style={{display:"flex",gap:10}}>
          <button onClick={onClose} style={{padding:"12px 20px",background:"none",border:"1px solid rgba(255,255,255,0.15)",color:"#889",borderRadius:8,cursor:"pointer",fontSize:15,minHeight:48}}>
...
function FolderView({folders,vendors,onCreateFolder,onDeleteFolder,onEditFolder,onOpenFolder,onAddVendorToFolder}) {
  ...
  return (
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:24}}>
        <div>
          <h2 style={{color:"#fff",fontFamily:"Georgia,serif",margin:"0 0 4px"}}>Problem Folders</h2>
          <div style={{fontSize:13,color:"#445"}}>Group fintechs by the problem they solve. Compare and present findings per folder.</div>
        </div>
      </div>
      ...
      {/* Folder grid */}
      {folders.length===0?(
        <div style={{textAlign:"center",padding:60,color:"#334"}}>
          <div style={{fontSize:40,marginBottom:12}}>📁</div>
          <div style={{fontSize:16,color:"#556"}}>No folders yet</div>
          <div style={{fontSize:13,color:"#334",marginTop:6}}>Create a folder above to start organizing fintechs by the problem they solve.</div>
        </div>
      ):(
                  ...
                  <div onClick={e=>e.stopPropagation()}>
                    ...
                    <div style={{display:"flex",gap:8}}>
                      ...
                      <button onClick={()=>setEditingId(null)}
                        style={{padding:"6px 14px",background:"none",border:"1px solid rgba(255,255,255,0.15)",borderRadius:6,color:"#889",cursor:"pointer",fontSize:13}}>Cancel</button>
                    </div>
                  </div>
                ):(
                  <>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
                      ...
                      <div style={{display:"flex",gap:6}}>
                        <button onClick={e=>{e.stopPropagation();setEditingId(folder.id);setEditName(folder.name);setEditDesc(folder.description||"");}}
                          style={{background:"none",border:"none",color:"#445",cursor:"pointer",fontSize:14,padding:"2px 6px"}}>✎</button>
                        <button onClick={e=>{e.stopPropagation();if(window.confirm(`Delete folder "${folder.name}"? Vendors inside will not be deleted.`))onDeleteFolder(folder.id);}}
                          style={{background:"none",border:"none",color:"#446",cursor:"pointer",fontSize:14,padding:"2px 6px"}}>✕</button>
                      </div>
                    </div>
                    <div style={{fontSize:16,fontWeight:700,color:"#fff",marginBottom:4}}>{folder.name}</div>
                    {folder.description&&<div style={{fontSize:12,color:"#556",marginBottom:10}}>{folder.description}</div>}
                    <div style={{display:"flex",gap:14,marginTop:8,paddingTop:10,borderTop:"1px solid rgba(255,255,255,0.06)"}}>
                      <div style={{textAlign:"center"}}>
                        <div style={{fontSize:20,fontWeight:700,color:"#00d4aa"}}>{folderVendors.length}</div>
                        <div style={{fontSize:10,color:"#445"}}>Vendors</div>
                      </div>
                      <div style={{textAlign:"center"}}>
                        <div style={{fontSize:20,fontWeight:700,color:"#4db8ff"}}>{avgScore}</div>
                        <div style={{fontSize:10,color:"#445"}}>Avg Score</div>
                      </div>
                      {topVendor&&(
                        <div style={{flex:1,textAlign:"right"}}>
                          <div style={{fontSize:11,color:"#445"}}>Top Pick</div>
                          <div style={{fontSize:13,color:"#fff",fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{topVendor.fintechName}</div>
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:24}}>
        <button onClick={onBack} style={{padding:"8px 14px",background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.1)",borderRadius:8,color:"#889",cursor:"pointer",fontSize:13}}>← Back</button>
        <div>
          <h2 style={{color:"#fff",fontFamily:"Georgia,serif",margin:"0 0 2px"}}>📁 {folder.name}</h2>
          {folder.description&&<div style={{fontSize:13,color:"#445"}}>{folder.description}</div>}
        </div>
        <div style={{marginLeft:"auto",fontSize:13,color:"#445"}}>{folderVendors.length} vendors</div>
      </div>

      {folderVendors.length===0?(
        <div style={{...css.card,textAlign:"center",padding:60}}>
          <div style={{fontSize:36,marginBottom:12}}>📭</div>
          <div style={{fontSize:15,color:"#556"}}>No vendors in this folder yet</div>
          <div style={{fontSize:13,color:"#334",marginTop:6}}>Log a fintech and assign it to this folder from the Basic tab.</div>
        </div>
      ):(
        <>
          {/* Score Summary Cards */}
            ...
            {sorted.map((v,i)=>{
              ...
              return (
                <div key={v.id} style={{...css.card,border:`1px solid ${i===0?"rgba(0,212,170,0.3)":"rgba(255,255,255,0.07)"}`,background:i===0?"rgba(0,212,170,0.05)":"rgba(255,255,255,0.03)"}}>
                  ...
                  <div style={{fontSize:14,fontWeight:700,color:"#fff",marginBottom:4,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{v.fintechName}</div>
          ...
          <div style={{...css.card,marginBottom:24,overflowX:"auto"}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
              <h3 style={{margin:0,fontSize:13,color:"#8899aa",textTransform:"uppercase",letterSpacing:"0.1em"}}>Head-to-Head Comparison</h3>
              <select value={sortBy} onChange={e=>setSortBy(e.target.value)} style={{...{background:"#0f1923",border:"1px solid rgba(255,255,255,0.1)",borderRadius:6,padding:"6px 10px",color:"#fff",fontSize:12}}}>
            ...
            <table style={{width:"100%",borderCollapse:"collapse",minWidth:500}}>
              <thead>
                <tr style={{borderBottom:"1px solid rgba(255,255,255,0.08)"}}>
                  <th style={{textAlign:"left",padding:"8px 10px",fontSize:11,color:"#446",textTransform:"uppercase"}}>Criteria</th>
                  {sorted.map(v=>(
                    <th key={v.id} style={{textAlign:"center",padding:"8px 10px",fontSize:13,color:"#fff",fontWeight:700,minWidth:100}}>
                      {v.fintechName}
                      <div style={{fontSize:10,color:"#556",fontWeight:400}}>{(v.effortAdj||0).toFixed(2)}</div>
                    </th>
                  ))}
                </tr>
              </thead>
              <tbody>
                ...
                {allCriteriaKeys.map(k=>{
                  ...
                  return (
                    <tr key={k} style={{borderBottom:"1px solid rgba(255,255,255,0.04)"}}>
                      <td style={{padding:"8px 10px",fontSize:12,color:"#8899aa"}}>{allCriteriaLabels[k]||k}</td>
                      {sorted.map((v,i)=>{
                        const val=v[k]||0;
                ...
                <tr style={{borderBottom:"1px solid rgba(255,255,255,0.04)",background:"rgba(255,255,255,0.02)"}}>
                  <td style={{padding:"8px 10px",fontSize:12,color:"#8899aa"}}>Implementation Effort</td>
                  {sorted.map(v=>{
                ...
                {/* Final score row */}
                <tr style={{borderTop:"2px solid rgba(255,255,255,0.1)",background:"rgba(0,212,170,0.04)"}}>
                  <td style={{padding:"10px 10px",fontSize:13,color:"#fff",fontWeight:700}}>Effort-Adj Score</td>
                  {sorted.map((v,i)=>(
          ...
          {/* Notes comparison */}
          <div style={css.card}>
            <h3 style={{margin:"0 0 16px",fontSize:13,color:"#8899aa",textTransform:"uppercase",letterSpacing:"0.1em"}}>Conference Notes Comparison</h3>
              ...
              return (
                <div key={key} style={{marginBottom:16,paddingBottom:16,borderBottom:"1px solid rgba(255,255,255,0.05)"}}>
                  ...
                  <div style={{display:"grid",gridTemplateColumns:`repeat(${sorted.length},1fr)`,gap:10}}>
                    {sorted.map(v=>(
                      <div key={v.id}>
                        <div style={{fontSize:11,color:"#556",marginBottom:3,fontWeight:600}}>{v.fintechName}</div>
...
export default function App() {
  ...
  const pColor={High:"#ff6b6b",Medium:"#ffd93d",Low:"#6bcb77"};
  const NAV=[["dashboard","Dashboard"],["list","Vendors"],["folders","Folders"],["trend","Trends"],["board","Board"],["weights","Weights"]];
  ...
  return (
    <div style={{minHeight:"100vh",background:"#080f18",color:"#c8d8e8",fontFamily:"'Segoe UI',sans-serif"}}>
      ...
      <div style={{maxWidth:1300,margin:"0 auto",padding:"24px 16px"}}>
                ...
                <div key={v.id} style={{...css.card,display:"grid",gridTemplateColumns:"30px 1fr auto auto auto auto",alignItems:"center",gap:12,padding:"13px 16px"}}>
                  ...
                  <div style={{minWidth:0}}>
                    ...
                    <div style={{display:"flex",gap:8,alignItems:"center",marginTop:2,fontSize:11,flexWrap:"wrap"}}>
                      ...
                      {v.notes&&<span style={{color:"#445",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:260}}>· {v.notes}</span>}
                    </div>
                  </div>
        ...
        {view==="folderDetail"&&(()=>{const f=folders.find(x=>x.id===activeFolderId||String(x.id)===String(activeFolderId));return f?<FolderDetailView folder={f} vendors={enriched} onBack={()=>setView("folders")} onEditVendor={v=>{setEditItem(v);setQuickMode(false);setShowForm(true);}} weights={weights} weightLabels={weightLabels} mustHaves={mustHaves}/>:<div style={{color:"#556",padding:40}}>Folder not found.</div>;})()}
      </div>
        ...
        <Modal onClose={()=>setDupeWarning(null)}>
          <div style={{textAlign:"center",padding:20}}>
            ...
            <div style={{display:"flex",gap:10,justifyContent:"center"}}>
              ...
              <button onClick={()=>{const snap={...dupeWarning,id:Date.now(),vendorWeights:dupeWarning.vendorWeights||weights,vendorWeightLabels:dupeWarning.vendorWeightLabels||weightLabels,vendorMustHaves:dupeWarning.vendorMustHaves||mustHaves};const next=[...vendors,snap];setVendors(next);persistVendors(next);setShowForm(false);setDupeWarning(null);}}
