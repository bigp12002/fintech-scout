...
// ─── Must-Have Rules Editor ───────────────────────────────────────────────────

function MustHaveEditor({mustHaves,onChange}) {
  const SCORE_FIELDS = Object.keys(DEFAULT_WEIGHTS);
  return (
    <div style={{marginTop:24,padding:16,background:"rgba(255,215,61,0.05)",border:"1px solid rgba(255,215,61,0.2)",borderRadius:10}}>
      <h4 style={{color:"#ffd93d",margin:"0 0 12px",fontSize:13,textTransform:"uppercase",letterSpacing:"0.08em"}}>Must-Have Rules</h4>
      <p style={{color:"#556",fontSize:12,marginBottom:14}}>Set the minimum score (1–5) any vendor must meet for each criterion to pass. Leave at 1 to disable.</p>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
        {SCORE_FIELDS.map(k=>(
          <div key={k} style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8}}>
            <span style={{fontSize:13,color:"#c8d8e8",flex:1}}>{WEIGHT_LABELS[k]}</span>
            <select value={mustHaves[k]||1} onChange={e=>onChange({...mustHaves,[k]:Number(e.target.value)})}
              style={{...css.selectDark,width:60,padding:"4px 8px",fontSize:13,color:mustHaves[k]>1?"#ffd93d":"#445"}}>
...
function WeightsEditor({weights,onChange,mustHaves,onMustHavesChange}) {
  const total=Object.values(weights).reduce((s,w)=>s+w,0);
  const setW=(k,v)=>onChange({...weights,[k]:Math.max(0,Math.min(100,Number(v)))});
  return (
    <div style={{maxWidth:680}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
        <h2 style={{color:"#fff",fontFamily:"Georgia,serif",margin:0}}>Scoring Weights</h2>
        <div style={{fontSize:13,color:Math.abs(total-100)<1?"#2ecc71":"#e74c3c",fontWeight:700}}>
          Total: {total}% {Math.abs(total-100)>=1&&"⚠ Must = 100%"}
        </div>
      </div>
      <p style={{color:"#556",fontSize:13,marginBottom:24}}>Adjust sliders or type values. Must total 100%.</p>
      {Object.keys(weights).map(k=>(
        <div key={k} style={{marginBottom:18}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
            <span style={{fontSize:14,color:"#c8d8e8"}}>{WEIGHT_LABELS[k]}</span>
            <div style={{display:"flex",alignItems:"center",gap:6}}>
              <input type="number" min="0" max="100" value={weights[k]} onChange={e=>setW(k,e.target.value)}
                style={{width:56,background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.15)",borderRadius:4,padding:"4px 8px",color:"#00d4aa",fontSize:13,fontWeight:700,textAlign:"center"}}/>
              <span style={{fontSize:12,color:"#445"}}>%</span>
            </div>
          </div>
          <input type="range" min="0" max="50" value={weights[k]} onChange={e=>setW(k,e.target.value)}
            style={{width:"100%",accentColor:"#00d4aa",cursor:"pointer",height:6}}/>
        </div>
      ))}
      <div style={{padding:14,background:Math.abs(total-100)<1?"rgba(0,212,170,0.05)":"rgba(231,76,60,0.05)",border:`1px solid ${Math.abs(total-100)<1?"rgba(0,212,170,0.2)":"rgba(231,76,60,0.2)"}`,borderRadius:8,marginBottom:8}}>
        <div style={{fontSize:13,color:Math.abs(total-100)<1?"#00d4aa":"#e74c3c",fontWeight:600}}>
          {Math.abs(total-100)<1?"✓ Weights balanced at 100%":`⚠ Total is ${total}% — adjust to reach 100%`}
        </div>
      </div>
      <MustHaveEditor mustHaves={mustHaves} onChange={onMustHavesChange}/>
    </div>
  );
}
...
function EntryForm({initial,onSave,onClose,departments,conferences,onAddDepartment,onAddConference,weights,mustHaves,quickMode=false}) {
      ...
      {tab==="scores"&&(
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"0 24px"}}>
          {Object.keys(DEFAULT_WEIGHTS).map(k=>(
            <RatingInput key={k} label={`${WEIGHT_LABELS[k]} (${weights[k]}%)`}
  ...
  return (
    <div style={{minHeight:"100vh",background:"#080f18",color:"#c8d8e8",fontFamily:"'Segoe UI',sans-serif"}}>
      ...
      <div style={{maxWidth:1300,margin:"0 auto",padding:"24px 16px"}}>
        ...
        {view==="dashboard"&&<>
          ...
          <div style={{...css.card,marginBottom:20}}>
            ...
            <div style={{display:"flex",gap:10,flexWrap:"wrap"}}>
              ...
              <button onClick={()=>exportJSON({vendors,departments,conferences,weights,mustHaves})} style={{padding:"10px 16px",background:"rgba(77,184,255,0.1)",border:"1px solid rgba(77,184,255,0.2)",borderRadius:8,color:"#4db8ff",cursor:"pointer",fontSize:13,minHeight:44}}>💾 Backup JSON</button>
            ...
            {enriched.filter(v=>!v.mustHave).length===0
              ...
              :<table style={{width:"100%",borderCollapse:"collapse"}}>
                ...
                <tbody>{enriched.filter(v=>!v.mustHave).map(v=>{
                  const failing=Object.entries(mustHaves).filter(([k,min])=>(v[k]||0)<min).map(([k])=>WEIGHT_LABELS[k]);
                  return (
                    <tr key={v.id}>
        ...
        {view==="weights"&&<WeightsEditor weights={weights} onChange={handleWeightsChange} mustHaves={mustHaves} onMustHavesChange={handleMustHavesChange}/>}
      </div>
      ...
      {showForm&&(
        <Modal onClose={()=>{setShowForm(false);setEditItem(null);}} wide>
          <EntryForm initial={editItem} onSave={saveVendor} onClose={()=>{setShowForm(false);setEditItem(null);}}
            ...
            weights={weights} mustHaves={mustHaves} quickMode={quickMode}/>
        </Modal>
      )}
